  ...
  up: async (queryInterface, Sequelize) => {
    ...
    */
    await queryInterface.sequelize.transaction(async transaction => {
      ...
      await queryInterface.addConstraint('demographics', {type: 'UNIQUE', fields: ['agency_id'], transaction});
      await queryInterface.addConstraint('demographics', {type: 'UNIQUE', fields: ['subdomain'], transaction});
      ...
      await queryInterface.sequelize.query(`CREATE TYPE enum_employments__roles AS ENUM ('BILLING', 'CONFIGURATION', 'PERSONNEL', 'REPORTING')`, {transaction});
      await queryInterface.sequelize.query(`ALTER TABLE employments ADD COLUMN roles enum_employments__roles[] NOT NULL DEFAULT '{}'`, {transaction});
      await queryInterface.addConstraint('employments', {type: 'UNIQUE', fields: ['demographic_id', 'user_id'], transaction});
      ...
      await queryInterface.renameColumn('agencies', 'data_set', 'data', {transaction})
      /// add a new state column with GNIS code, to become a new foreign key to simplified state model
      await queryInterface.addColumn('agencies', 'state', Sequelize.STRING, {transaction});
      /// bulk update the column with the appropriate value
      await queryInterface.sequelize.query(`UPDATE agencies SET state=states.code FROM states WHERE agencies.state_id=states.id`, {transaction});
      /// now drop the old foreign key
      await queryInterface.removeColumn('agencies', 'state_id', {transaction});
      /// rename state to new state_id foreign key
      await queryInterface.renameColumn('agencies', 'state', 'state_id', {transaction})
      ...
      await queryInterface.sequelize.query(`UPDATE agencies SET created_by_id=users.id, updated_by_id=users.id FROM users WHERE users.is_admin=TRUE`, {transaction});
      ...
      await queryInterface.renameColumn('states', 'code', 'id', {transaction});
      /// make it the new primary key
      await queryInterface.addConstraint('states', {type: 'PRIMARY KEY', fields: ['id'], transaction});
      /// remove the now redundant unique index
      await queryInterface.removeIndex('states', 'states_code', {transaction});
      /// add a flag for easy check if the state config has been loaded
      await queryInterface.addColumn('states', 'is_configured', {type: Sequelize.BOOLEAN, allowNull: false, defaultValue: false}, {transaction});
      ...
      await queryInterface.addConstraint('agencies', {type: 'FOREIGN KEY', fields: ['state_id'], references: {table: 'states', field: 'id'}, transaction});
      /// and not null
      ...
      await queryInterface.addColumn('agencies', 'demographic_id', {
          type: Sequelize.UUID,
          references: {
            model: {
              tableName: 'demographics'
      ...
      await queryInterface.addColumn('agencies', 'state_agency_id', {
          type: Sequelize.UUID,
          references: {
            model: {
              tableName: 'agencies'
      ...
      await queryInterface.sequelize.query(`ALTER TABLE agencies ADD CONSTRAINT agencies_state_uk EXCLUDE (state_unique_id WITH =, number WITH =, state_id WITH =) WHERE (demographic_id IS NULL) DEFERRABLE INITIALLY DEFERRED`, {transaction});
      await queryInterface.sequelize.query(`ALTER TABLE agencies ADD CONSTRAINT agencies_demographic_uk EXCLUDE (demographic_id WITH =, state_unique_id WITH =, number WITH =, state_id WITH =) WHERE (demographic_id IS NOT NULL)`, {transaction});
    });
  },
