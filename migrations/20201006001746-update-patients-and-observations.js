    ...
    await queryInterface.sequelize.transaction(async (transaction) => {
      ...
      for (const table of ['patients', 'patient_observations']) {
        const [
          ...
          `SELECT id, blood_pressure FROM ${table}`,
          { transaction }
        );
        for (const result of results) {
          ...
          if (m) {
            ...
            if (m1 > m2) {
              await queryInterface.sequelize.query(
                `UPDATE ${table} SET bp_systolic=${m1}, bp_diastolic=${m2} WHERE id='${result.id}'`,
                { transaction }
              );
            } else {
              await queryInterface.sequelize.query(
                `UPDATE ${table} SET bp_systolic=${m2}, bp_diastolic=${m1} WHERE id='${result.id}'`,
                { transaction }
              );
            }
          }
      ...
      for (const result of results) {
        ...
        await queryInterface.sequelize.query(
          `UPDATE patient_observations SET updated_attributes='${JSON.stringify(
