...
export class AgencyComponent {
  ...
  search = (text$: Observable<string>) =>
    text$.pipe(
      ...
      switchMap(term =>
        this.api.agencies.index(new HttpParams().set('search', term).set('stateId', this.stateId))
          .pipe(
            catchError(() => of([])),
            map(res => res.body)
          )
  ...
  onNext() {
    if (this.agency && !this.isLoading) {
      this.isLoading = true;
      if (this.agency.id) {
        this.api.agencies.demographic(this.agency.id)
          .pipe(
            catchError(res => {
              this.isLoading = false;
              this.navigation.goTo('/url', {stateId: this.stateId, agencyId: this.agency.id});
              return empty();
            })
          )
          .subscribe(res => {
            ...
            this.navigation.goTo('/exists', {stateId: this.stateId, agencyId: this.agency.id});
          });
      } else {
        this.navigation.goTo('/notify', {reason: 'agency', state: this.app.state?.name, agency: this.agency});
      }  
    }
  }
}
