...
export class BaseDemographicsComponent {
  ...
  protected sectionSchemaPath: string;
  protected sectionSchemaRootElementName: string;
  protected sectionColumns: any[];
  ...
  onSubmit() {
    let request;
    if (this.isNewRecord) {
      request = this.api.demographics[this.section].create(this.record);
    } else {
      const id = this.record._attributes?.UUID;
      if (id) {
        request = this.api.demographics[this.section].update(id, this.record);
      } else {
        request = this.api.demographics[this.section].update(this.record);
      }
    }
