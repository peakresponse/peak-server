...
export class ScenePatientsComponent implements OnDestroy {
  ...
  filter(tabIndex: number) {
    ...
    this.router.navigate(['/scenes', this.scene.id, 'patients'], {fragment, queryParamsHandling: "preserve"});
  }

  sort(sort: string) {
    this.router.navigate(['/scenes', this.scene.id, 'patients'], {queryParams: {sort}, preserveFragment: true});
  }

  show(patient: any) {
    this.router.navigate([{outlets: {modal: ['patients', patient.id]}}], {relativeTo: this.route, preserveFragment: true, queryParamsHandling: "preserve"});
  }
}
