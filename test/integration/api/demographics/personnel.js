...
describe('/api/demographics/personnel', function() {
  let testSession;
  ...
  describe('POST /invite', function() {
    it('creates and sends bulk invitations', async function() {
      await testSession.post('/api/demographics/personnel/invite')
        .set('Host', `bmacc.${process.env.BASE_HOST}`)
        .send({
      ...
      const employments = await models.Employment.findAll({where: {
    ...
    it('allows blank/empy name fields', async function() {
      await testSession.post('/api/demographics/personnel/invite')
        .set('Host', `bmacc.${process.env.BASE_HOST}`)
        .send({
          message: 'Test message',
  ...
  describe('POST /accept', function() {
    it('creates a new user and associates with agency employment by matching email', async function() {
      const response = await testSession.post('/api/demographics/personnel/accept')
        .set('Host', `bmacc.${process.env.BASE_HOST}`)
        .send({
          agencyId: '',
          invitationCode: '',
    ...
    it('creates a new user and associates with agency employment by matching invitation code', async function() {
      const response = await testSession.post('/api/demographics/personnel/accept')
        .set('Host', `bmacc.${process.env.BASE_HOST}`)
        .send({
          agencyId: '',
    ...
    it('returns not found for an invalid invitation code', async function() {
      const response = await testSession.post('/api/demographics/personnel/accept')
        .set('Host', `bmacc.${process.env.BASE_HOST}`)
        .send({
          agencyId: '',
    ...
    it('creates a new user and associates with agency employment pending for unmatched email', async function() {
      const response = await testSession.post('/api/demographics/personnel/accept')
        .set('Host', `bmacc.${process.env.BASE_HOST}`)
        .send({
          agencyId: '',
      ...
      const employment = await models.Employment.findOne({where: {
