  ...
  describe('Contact', function() {
    ...
    beforeEach(async function() {
      ...
      await models.sequelize.transaction(async transaction => {
        dAgency = await models.DemAgency.register(user, agency, "bbfpd", {transaction});
      });
    });

    describe('.save()', function() {
      it('populates columns from the data', async function() {
        const contact = models.Contact.build();
        contact.agencyId = dAgency.id;
      ...
      it('validates its data against NEMSIS xsd', async function() {
        const contact = models.Contact.build();
        contact.agencyId = dAgency.id;
        contact.createdById = user.id;
        contact.updatedById = user.id;
        ...
        await assert.rejects(contact.save(), error => {
            ...
            assert(error instanceof models.Sequelize.ValidationErrorItem);
            error = error.original;
            assert(_.find(error.errors, {path: 'dContact.02', message: 'This is not a valid value.', value: ''}));
            assert(_.find(error.errors, {path: 'dContact.10', message: 'This is not a valid value.', value: '4155551234'}));
            return true;
          });
      });
    });
