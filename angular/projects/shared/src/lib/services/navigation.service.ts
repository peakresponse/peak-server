...
export class NavigationService {
  ...
  goTo(url: string, queryParams: any = null, fragment?: string, replaceUrl: boolean = false) {
    this.router.navigate([url], { queryParams, fragment, replaceUrl });
  }

  replaceWith(url: any, queryParams: any = null, fragment?: string) {
  ...
  backTo(url: string) {
    if (this.previousUrl == url) {
      ...
      this.router.navigate([`/${urlComponents.root.children[PRIMARY_OUTLET].toString()}`], {
        ...
        fragment: urlComponents.fragment || undefined,
      });
    }
  }
}
