...
router.post('/invite', interceptors.requireAgency(models.Employment.Roles.PERSONNEL), helpers.async(async function(req, res, next) {
...
router.post('/accept', helpers.async(async function(req, res, next) {
  ...
  try {
    ...
    await models.sequelize.transaction(async transaction => {
      ...
      if (invitationCode) {
        ...
        employment = await models.Employment.findOne({where: {
