...
function versions() {
  const { stdout } = shelljs.exec(`cd ${basePath}/master; git ls-remote --refs --sort=-v:refname`, {
    silent: true,
  });
  return stdout
    .trim()
    .split('\n')
    .map((v) => v.match(/refs\/tags\/(\d+\.\d+\.\d+\..+)/)?.[1])
    .filter(Boolean)
...
class NemsisPublicRepo {
  ...
  emsTranslationXsltPath(fromVersion, toVersion) {
    if (fromVersion.startsWith('3.5.0') && toVersion.startsWith('3.4.0')) {
      return this.translationPath(`v3.5.0 to v3.4.0/EMS-v350_to_v340.xslt`);
    }
    return null;
  }
  ...
  translationPath(filename) {
    return path.resolve(__dirname, `../../${basePath}`, this.nemsisVersion, 'Translation', filename);
  }
