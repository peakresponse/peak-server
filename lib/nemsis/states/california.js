  ...
  for (const row of data.rows) {
    ...
    if (row['Hospital Designation']) {
      ...
      for (const name of names) {
        if (name !== '') {
          const value =
            CommonTypes.enums.HospitalDesignation.nameMapping[name] ||
    ...
    setValue(
      ...
      true,
      row[
        'National Provider Identifier (comma separate for multiple. Max length allowed per code is 10 character. '
      ],
      true
    );
    setValue(
...
const appendAgenciesFromSpreadsheet = (dataSet) => {
  ...
  return new Promise((resolve, reject) => {
    fetch(
      `https://emsa.ca.gov/wp-content/uploads/sites/71/2020/09/201_CEMSIS_Provider_Agency_List-09.04.2020.xlsx`
    )
      .then((res) => {
    ...
    .then((data) => {
      ...
      for (const row of data.rows) {
        ...
        if (
          _.find(dataSet.json.StateDataSet.sAgency.sAgencyGroup, agency) ===
          undefined
        ) {
      ...
      dataSet.json.StateDataSet.sAgency.sAgencyGroup = dataSet.json.StateDataSet.sAgency.sAgencyGroup.concat(
        sAgencyGroup
      );
      return { sAgencyGroup };
    })
...
const processStateRepoFiles = async (tmpDir, files, dataSet) => {
  ...
  for (const filePath of files) {
    if (
      ...
      facilitySpreadsheet = await parseSpreadsheet(
  ...
  if (facilitySpreadsheet) {
    ...
    dataSet.json.StateDataSet.sFacility = await parseFacilitySpreadsheet(
      facilitySpreadsheet
    );
  }
