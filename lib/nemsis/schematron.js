...
async function validate(doc, xml, schXslPath) {
  ...
  try {
    ...
    let xsltSucceeded = false;
    try {
      execFileSync('bin/xslt', [srcFileObj.name, schXslPath, dstFilePath], { stdio: 'ignore' });
      xsltSucceeded = true;
    } catch {
      xsltSucceeded = false;
    }
    if (xsltSucceeded) {
      ...
      if (doc) {
        ...
        for (const failedAssert of failedAsserts) {
          ...
          const role = failedAssert._attributes?.role;
          let level;
          if (role?.match(/\[[A-Z]+\]/)) {
            level = role.substring(1, role.length - 1).toLowerCase();
          }
