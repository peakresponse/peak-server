...
const getStateRepoFiles = (repo) => {
  return fetch(`${API_BASE_URL}/projects/NES/repos/${repo}/files?limit=100`).then((res) => res.json());
};
...
const validateSchema = async (xsdPath, rootTag, groupTag, rawData) => {
  ...
  try {
    ...
    const errors = [];
    for (const m of err.message.matchAll(/element ([^:]+): Schemas validity error : Element '[^']+': '([^']*)' is not a valid value/g)) {
