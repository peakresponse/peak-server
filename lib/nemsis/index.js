...
const getStateRepos = () => {
  return fetch(`${API_BASE_URL}/projects/NES/repos?limit=100`).then((res) =>
    res.json()
  );
};
...
const getStateRepoFiles = (repo) => {
  return fetch(
    `${API_BASE_URL}/projects/NES/repos/${repo}/files?limit=100`
  ).then((res) => res.json());
};
...
const parseStateDataSet = (filePath) => {
  return new Promise((resolve, reject) => {
    ...
    const m = json.StateDataSet._attributes['xsi:schemaLocation'].match(
      /nemsis_v\d\/(\d+\.\d+\.\d+)\.([^/]+)\//
    );
    if (m) {
      if (m[1].startsWith('3.4')) {
        ...
        if (
          shelljs.exec(
            `bin/xslt ${filePath} nemsis/xslt/STATE-v340_to_v350.xslt ${destFilePath}`
          ).code === 0
        ) {
...
const validateSchema = async (xsdPath, rootTag, groupTag, rawData) => {
  ...
  const individualXsdPath = path.resolve(
    __dirname,
    '../../nemsis/xsd/wrappers',
    xsdPath
  );
  ...
  try {
    ...
    _.set(rawData, ['_attributes', 'pr:isValid'], true);
    return null;
  } catch (err) {
    /// TODO parse error for specified field validation messages to return
    const errors = [];
    for (const m of err.message.matchAll(
      /element ([^:]+): Schemas validity error : Element '[^']+': '([^']*)' is not a valid value/g
    )) {
      errors.push(
    ...
    _.set(rawData, ['_attributes', 'pr:isValid'], false);
