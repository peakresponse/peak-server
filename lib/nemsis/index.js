...
const reorderData = function(element, data) {
  ...
  if (elements) {
    ...
    for (let element of elements) {
      ...
      if (name) {
        ...
        if (value) {
          reorderedData[name] = reorderData(element, value);
        }
      }
    }
...
const validateSchema = async function(xsdPath, rootTag, groupTag, data) {
  ...
  _.assign(doc[rootTag]._attributes, {
    ...
    'xsi:schemaLocation': 'http://www.nemsis.org https://nemsis.org/media/nemsis_v3/3.5.0.191130CP1/XSDs/NEMSIS_XSDs/DEMDataSet_v3.xsd'
  });
  ...
  xsdPath = path.resolve(__dirname, '../../nemsis/xsd', xsdPath);
  /// open the xsd
  const xsd = xmljs.xml2js(fs.readFileSync(xsdPath).toString(), {compact: true});
  ...
  try {
    ...
    _.set(data, ['_attributes', 'pr:isValid'], true);
    return null;
  } catch (err) {
    /// TODO parse error for specified field validation messages to return
    if (process.env.NODE_ENV != 'test') {
      console.log('!!!', err.message, xml);
    }
    const errors = []
    for (let m of err.message.matchAll(/element ([^:]+): Schemas validity error : Element '[^']+': '([^']*)' is not a valid value/g)) {
      errors.push(new Sequelize.ValidationErrorItem('This is not a valid value.', 'Validation error', m[1], m[2]))
    }
    _.set(data, ['_attributes', 'pr:isValid'], false);
