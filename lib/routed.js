...
async function upsertVenue(venueId) {
  ...
  const data = {
    ...
    state: venue.stateId,
    isMfaEnabled: false,
    isActive: venue.archivedAt === null,
  };
  const credentials = await authenticate(venue.region);
  await venue.region.update({ routedCredentials: credentials });
...
async function assignAgencyToVenue(venueId, agencyId) {
  const venue = await models.Venue.findByPk(venueId, {
    include: ['region'],
  });
  if (!venue || !venue.region?.routedUrl) {
  ...
  const data = {
    state: agency.stateId,
    stateUniqueId: agency.stateUniqueId,
  };
  const credentials = await authenticate(venue.region);
  await venue.region.update({ routedCredentials: credentials });
  const url = new URL(`/api/organizations/${venueId}/assign`, venue.region.routedUrl);
  const response = await fetch(url, {
    method: 'POST',
    headers: {
      Accept: 'application/json',
...
async function upsertFacility(facilityId) {
  const facility = await models.Facility.findByPk(facilityId);
  if (!facility.venueId) {
    return Promise.resolve();
  }
  const venue = await models.Venue.findByPk(facility.venueId, {
    include: ['region', 'facilities'],
  });
  if (!venue || !venue.region?.routedUrl) {
    return Promise.resolve();
  }
  const data = {
    id: facilityId,
    OrganizationId: facility.venueId,
    name: facility.name,
    state: facility.stateId,
    stateFacilityCode: facility.id,
    sortSequenceNumber: venue.facilities.findIndex((f) => f.id === facilityId) + 1,
    isActive: facility.archivedAt === null,
    customInventory: facility.inventory,
  };
  const credentials = await authenticate(venue.region);
  await venue.region.update({ routedCredentials: credentials });
  const url = new URL(`/api/hospitals/${facilityId}`, venue.region.routedUrl);
  const response = await fetch(url, {
    method: 'PUT',
    headers: {
      Accept: 'application/json',
